...
def find_stops_for_chunk(first_chunk_candidates_accepted,
    ...
    print(dists_travelled.shape)
    dists_to_go = distance_matrix[chunk_candidates, 1]  # distance from end point to candidate
    dists_to_go = dists_to_go.reshape((len(dists_to_go), 1))
...
i=2
first_chunk_candidates_accepted = stops_dict_a_chunk['chunk' + str(i-1)]
...
# between 5 start locs, 64 locations (some may be duped but that's fine)
